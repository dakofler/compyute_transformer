...
class TransformerBlock(Module):
    ...
    def __init__(
        ...
        self.attn = MultiHeadAttention(
            in_channels, n_heads, mask, dropout, out_scale, bias, dtype
        )
        self.dropout1 = Dropout(dropout)

        self.ln2 = LayerNorm((in_channels,), dtype=dtype)
        self.ffwd = FeedForward(in_channels, ffwd_channels, out_scale, bias, dtype)
    ...
    @Module.register_backward
    def backward(self, dy: Tensor) -> Tensor:
        dy = dy + self.ln2.backward(self.ffwd.backward(self.dropout2.backward(dy)))
